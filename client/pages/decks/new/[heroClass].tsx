...
const NewDeck: NextPage = () => {
  ...
  useEffect(() => {
    if (heroClass && activePage > 0)
      fetch(
        `http://localhost:3000/hearthstone/cards?page=${activePage}&cardClass=${heroClass}&mode=${mode}`
      )
        .then((res) => res.json())
        .then((data) => {
          setCards(data?.cards);
              ...
              <Col span={3}>
                <Card withBorder={true}>
                  ...
                  <Card.Section withBorder={true} px={"sm"} py={"sm"}>
                    ...
                    />
                    <Button
                      onClick={() => getDeckCode(deckCards, hsClass)}
                      mr={"sm"}
                      variant="gradient"
        ...
        <Grid>
          ...
          <Col offsetSm={0} span={1} style={{ position: "relative" }}>
            <div style={{ bottom: 8, position: "absolute" }}>
              <ActionIcon size={36} variant="filled">
                <IconSearch size={"18"} />
              </ActionIcon>
            </div>
          </Col>
